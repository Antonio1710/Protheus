    ...
    BeginSQL Alias "TRB"
			...
			SELECT  FONTES.ZK_PLACA,
			  ...
			  FROM (SELECT SZK.ZK_DTENTR,
							...
							(SELECT CASE WHEN SUM(SZI.ZI_VALOR) IS NULL THEN 0 ELSE SUM(SZI.ZI_VALOR) END FROM %Table:SZI%  SZI WHERE SZK.ZK_PLACA = SZI.ZI_PLACA AND SZI.ZI_TIPO = 'A' AND SZK.ZK_DTENTR = SZI.ZI_DATAROT AND SZK.ZK_ROTEIRO = SZI.ZI_ROTEIRO AND SZI.D_E_L_E_T_ <> '*') AS ZK_ACRESCIMO,
							(SELECT CASE WHEN SUM(SZI.ZI_VALOR) IS NULL THEN 0 ELSE SUM(SZI.ZI_VALOR) END FROM %Table:SZI%  SZI WHERE SZK.ZK_PLACA = SZI.ZI_PLACA AND SZI.ZI_TIPO = 'D' AND SZK.ZK_DTENTR = SZI.ZI_DATAROT AND SZK.ZK_ROTEIRO = SZI.ZI_ROTEIRO AND SZI.D_E_L_E_T_ <> '*') AS ZK_DECRESCIMO,
							SZK.ZK_PESOL,
							SZK.ZK_PBRUTO,
							SZK.ZK_KMENT,
							SZK.ZK_ENTREGA
						...
						LEFT JOIN %Table:SZG% SZG
								...
								AND SZG.D_E_L_E_T_ <> '*'
					WHERE SZK.ZK_DTENTR   >= %EXP:cDataIni%
