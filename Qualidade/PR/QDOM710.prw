...
U_ADINF009P(SUBSTRING(ALLTRIM(PROCNAME()),3,LEN(ALLTRIM(PROCNAME()))) + '.PRW',SUBSTRING(ALLTRIM(PROCNAME()),3,LEN(ALLTRIM(PROCNAME()))),'Transfere os valores das var. de memoria para var. do word')

//Variaveis do .DOT Padrao
OLE_SetDocumentVar( oWord, "Adv_NomeFilial"        , cNomFilial )                                            	//01 - Filial do Sistema
OLE_SetDocumentVar( oWord, "Adv_CopiaControlada"   , cTpCopia )                                              	
...
OLE_SetDocumentVar( oWord, "Adv_CodAss1",            M->QDH_CODASS, .t.  )                     
//26 - Assunto ( Codigo )
