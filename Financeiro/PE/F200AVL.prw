...
/*/{Protheus.doc} User Function F200AVL
    ...
    @history chamado 056247 - FWNM - 21/05/2020 - Melhoria para liberar crédito e estoque automaticamente quando C5_XWSPAGO = S
    ...
    @history chamado 059688 - FWNM - 22/07/2020 - || OS 061190 || FINANCEIRO || YEDA || 93029-5663 || PEDIDOS ANTECIPADOS
    @history chamado 059415 - FWNM - 23/07/2020 - || OS 060907 || FINANCAS || WAGNER || 11940283101 || WS BRADESCO
    @history chamado 060029 - FWNM - 27/07/2020 - || OS 060907 || FINANCAS || WAGNER || 11940283101 || WS BRADESCO
    @history chamado 060029 - FWNM - 28/07/2020 - || OS 060907 || FINANCAS || WAGNER || 11940283101 || WS BRADESCO
    @history chamado TI     - FWNM - 03/08/2020 - E1_IDCNAB Duplicado
    @history ticket 102 - FWNM - 18/08/2020 - WS Bradesco - Gravar C5_XAPREAPR=L quando C5_XWSPAGO=S
    @history ticket 102 - FWNM - 28/08/2020 - WS Bradesco - FIE deletados sem explicação, mesmo tendo C5 e E1
    @history ticket 745 - FWNM - 21/09/2020 - Implementação título PR
...
User Function F200AVL()
    ...
    Local cCodRet69  := GetMV("MV_#WSOC69",,"69") // chamado 059688 - FWNM - 22/07/2020 - || OS 061190 || FINANCEIRO || YEDA || 93029-5663 || PEDIDOS ANTECIPADOS
    ...
    // ticket 745 - FWNM - Implementação título PR - 21/09/2020
    Local cTipoE1    := GetMV("MV_#WSTIPO",,"PR") 
    Local lBxPROk    := .f.
    //

    // E1_IDCNAB // @history ticket 745 - FWNM - 08/10/2020 - Retorno CNAB Cobrança - Título não encontrado
    If AllTrim(cIDCNAB) <> AllTrim(cKey3SE1WS)
        cIDCNAB := cKey3SE1WS
    EndIf

    // E1_IDCNAB Duplicado - Chamado TI - FWNM - 03/08/2020
    ...
    If AllTrim(cBanco) == "237" .and. cOcorre == "06" // Bradesco WS
        ...
        If !Empty(AllTrim(cIDCNAB))
            ...
            If SE1->( dbSeek(cIDCnab) )
                ...
                //If AllTrim(SE1->E1_TIPO) == "RA" .and. !Empty(SE1->E1_XWSBRAC) .and. ( Val(AllTrim(SE1->E1_XWSBRAC)) == Val(AllTrim(cCodRetOk)) .or. Val(AllTrim(SE1->E1_XWSBRAC)) == Val(AllTrim(cCodRet69)) ) // chamado 059688 - FWNM - 22/07/2020 - || OS 061190 || FINANCEIRO || YEDA || 93029-5663 || PEDIDOS ANTECIPADOS
                If AllTrim(SE1->E1_TIPO) == AllTrim(cTipoE1) .and. !Empty(SE1->E1_XWSBRAC) .and. ( Val(AllTrim(SE1->E1_XWSBRAC)) == Val(AllTrim(cCodRetOk)) .or. Val(AllTrim(SE1->E1_XWSBRAC)) == Val(AllTrim(cCodRet69)) )  // ticket 745 - FWNM - 21/09/2020 - Implementação título PR
                    ...
                    */
                        ...
                        //If SC5->( dbSeek(SE1->E1_FILIAL+FIE->FIE_PEDIDO) ) // @history ticket 102 - FWNM - 28/08/2020 - WS Bradesco - FIE deletados sem explicação, mesmo tendo C5 e E1

                            If AllTrim(Posicione("SE4",1,FWxFilial("SE4")+SC5->C5_CONDPAG,"E4_CTRADT")) == "1" // Cond Adiantamento = SIM // @history ticket 102 - FWNM - 28/08/2020 - WS Bradesco - FIE deletados sem explicação, mesmo tendo C5 e E1
                            
                                lProcessa := .f. //chamado 060029 - FWNM - 28/07/2020 - || OS 060907 || FINANCAS || WAGNER || 11940283101 || WS BRADESCO
                                
                                // ticket 745 - FWNM - 21/09/2020 - Implementação título PR
                                ...
                                If lBxPROk
                                    ...
                                    logZBE(SC5->C5_NUM + " GRAVADO CAMPO C5_XWSPAGO=S PELO RETORNO CNAB ARQ " + AllTrim(MV_PAR04)) // Chamado n. 056247 || OS 057671 || FINANCEIRO || LUIZ || 8451 || BOLETO BRADESCO WS - FWNM - 21/05/2020
                                    ...
                                    If SC9->( dbSeek(SC5->(C5_FILIAL+C5_NUM)) )
                                        Do While SC9->( !EOF() ) .and. SC9->C9_FILIAL==SC5->C5_FILIAL .and. SC9->C9_PEDIDO==SC5->C5_NUM // Chamado n. 056247 || OS 057671 || FINANCEIRO || LUIZ || 8451 || BOLETO BRADESCO WS - FWNM - 27/05/2020
                                            ...
                                            SC9->( dbSkip() )
                                        EndDo
                                        logZBE(SC5->C5_NUM + " FOI LIBERADO CREDITO/ESTOQUE PELO RETORNO CNAB ROTINA PADRAO A450GRAVA") // Chamado n. 056247 || OS 057671 || FINANCEIRO || LUIZ || 8451 || BOLETO BRADESCO WS - FWNM - 21/05/2020
                                    EndIf
                                    //
        ...
        If !lAchouTit
            ...
            If SE1->( dbSeek( Work->(E1_FILIAL+E1_PREFIXO+E1_NUM+E1_PARCELA+E1_TIPO) ) )
                ...
                If AllTrim(SE1->E1_TIPO) == AllTrim(cTipoE1) .and. !Empty(SE1->E1_XWSBRAC) .and. ( Val(AllTrim(SE1->E1_XWSBRAC)) == Val(AllTrim(cCodRetOk)) .or. Val(AllTrim(SE1->E1_XWSBRAC)) == Val(AllTrim(cCodRet69)) ) // Ticket 745 - Implementação substituição PR x RA no PV de adiantamento - 21/09/2020
                    ...
                    */
                        ...
                        SC5->( dbSetOrder(1) ) // C5_FILIAL+C5_NUM
                        If SC5->( dbSeek(SE1->E1_FILIAL+cFIEPEDIDO) ) 
                        //If SC5->( dbSeek(SE1->E1_FILIAL+FIE->FIE_PEDIDO) ) // Cond Adiantamento = SIM // @history ticket 102 - FWNM - 28/08/2020 - WS Bradesco - FIE deletados sem explicação, mesmo tendo C5 e E1
            ...
            If !lAchouTit
                lProcessa := .f. // deixar falso qdo não encontrar... // chamado 059415 - FWNM - 23/07/2020 - || OS 060907 || FINANCAS || WAGNER || 11940283101 || WS BRADESCO
                // chamado 060029 - FWNM - 27/07/2020 - || OS 060907 || FINANCAS || WAGNER || 11940283101 || WS BRADESCO
                RestArea( aAreaSE1 )
                RestArea( aAreaSC5 )
                RestArea( aAreaFIE )
                //
    ...
    // chamado 060029 - FWNM - 27/07/2020 - || OS 060907 || FINANCAS || WAGNER || 11940283101 || WS BRADESCO    
    /*
    If !lAchouTit // chamado 059415 - FWNM - 23/07/2020 - || OS 060907 || FINANCAS || WAGNER || 11940283101 || WS BRADESCO
        lProcessa := .f. // deixar falso qdo não encontrar... // chamado 059415 - FWNM - 23/07/2020 - || OS 060907 || FINANCAS || WAGNER || 11940283101 || WS BRADESCO
        RestArea( aAreaSE1 )
        RestArea( aAreaSC5 )
        RestArea( aAreaFIE )
